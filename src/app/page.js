...
//         console.log("🛑 Automated Scout: Finished checking all cameras.");
...
//       try {
//         console.log(
//           `📷 Automated Scout: Checking ${currentCamera.nodeName}...`,
//         );
...
//         // FIX: Safely extract the text whether Gradio returns a string or an object
//         const rawOutput =
//           typeof prediction === "string" ? prediction : prediction?.label || "";

//         console.log(
//           `🤖 Scout AI Output for ${currentCamera.nodeName}:`,
//           rawOutput,
//         );

//         const detectedLabel = rawOutput.toLowerCase();

//         // STRICTER CHECK: Look specifically for the word "unhealthy"
...
//           setSickPlants((prev) => [
//             ...new Set([...prev, currentCamera.plantIndex]),
//           ]);
//         }
...
//                     onClick={() => setSelectedPlant(alert.node)}
//                   />
//                 ))
//               )}
//             </div>
...
//     <div
//       onClick={onClick}
//       className="flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer bg-red-50 border-red-100"
//     >
...
export default function Home() {
    ...
    const scoutInterval = setInterval(async () => {
      ...
      try {
        ...
        if (detectedLabel.includes("unhealthy")) {
          ...
          // FIX: Correctly check `plantIndex` instead of `index`
          setSickPlants((prev) => {
  ...
  const handlePlantClick = (clickedIndex) => {
    // FIX: If the user clicked the ground, ignore it.
    if (clickedIndex === undefined) return;

    // FIX: Look for `plantIndex` to match the data structure perfectly
    const sickData = sickPlants.find((p) => p.plantIndex === clickedIndex);

    if (sickData) {
      setSelectedPlantInfo({
        id: sickData.nodeName,
        autoImage: sickData.file,
      });
    } else {
      setSelectedPlantInfo({
                ...
                alerts.map((alert) => (
                  <AlertItem
                    ...
                    onClick={() =>
                      setSelectedPlantInfo({
                        id: alert.node,
                        autoImage: sickPlants.find(
                          (p) => p.nodeName === alert.node,
                        )?.file,
                      })
                    }
                  />
